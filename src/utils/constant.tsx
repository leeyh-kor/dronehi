export const items = [
  {
    question:
      "1.	신라의 왕인 당신은 백제와 연합하여 고구려와 싸워 이겼습니다. 기존에 한강 유역을 나누어 갖기로 하였지만 한강 유역을 온전히 차지하면 나라에 큰 이점이 있다는 것을 알고 있습니다. 어떻게 하겠습니까?",
    answer1: "동맹을 맺었으니 끝까지 의리를 지켜야지! 나누어 갖자!",
    answer1Code: "J",
    answer2: "그래도 나의 나라를 지키는 게 먼저지. 저들은 언제 적이 될지 몰라!",
    answer2Code: "P",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "2.	신라시대에 살고 있는 당신은 어느 날 밤에 산에 올라앉아 오줌을 누니 서울에 가득차는 꿈을 꾸었습니다. 아침에 일어나 동생에게 꿈을 얘기해주었더니, 동생이 비단치마를 주고 “내가 그 꿈을 사겠다고” 말했습니다. 당신은 어떤 반응을 보이겠습니까?",
    answer1: "오 비단치마 완전 이득! 당장 팔께!",
    answer1Code: "S",
    answer2: "b.	비단치마를 주고 산다고..? 좋은 꿈인가? 일단 안 팔아야지!",
    answer2Code: "N",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "3.	조선시대 수군인 당신은 나라를 위해 싸워 공을 세웠는데 당신의 세력이 강해지는 것을 두려워 한 신하들이 죄를 씌워 옥중에 가두고 고문을 하였습니다. 나라의 위기가 있자 당신에게 다시 복직을 할 것을 요구하였습니다. 어떻게 하겠습니까?",
    answer1:
      "내가 없으면 싸움이 질 텐데.. 그럼 나라가 망하니 일단 복직해서 싸우자!",
    answer1Code: "T",
    answer2: "또 무슨 짓을 할 줄 알고, 절대 복직 안해!",
    answer2Code: "F",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "4. 당신은 고려시대 행사인 연등회에 참석하였습니다. 개경 거리 곳곳에 등불이 밝혀져 있고 축제 분위기가 무르익었습니다. 당신 또한 사람들과 함께 어울리며 대화를 나누고 있습니다. 그러던 중 대화가 갑자기 끊겼습니다.",
    answer1: "분위기를 다시 살려야지! 라고 생각하며 대화를 이끌어 나간다.",
    answer1Code: "E",
    answer2:
      "누군가가 얘기하겠지! 라고 생각하며 다른 사람들의 반응을 기다린다.",
    answer2Code: "I",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "5.	당신은 조선시대 집현전 학자입니다. 왕께서 내일까지 유교 경서에 대한 서적을 써오라고 하였습니다. 그러던 중 유학에서 돌아온 친구가 주막에서 한 잔 하자고 불렀습니다. 어떻게 하겠습니까?",
    answer1: "간만에 친구가 왔으니 일단 놀고 내일 생각해야겠다!",
    answer1Code: "P",
    answer2: "오늘 논다면 내일까지 못 끝낼 수도 있는데.. 다음에 놀아야겠다!",
    answer2Code: "J",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "6.	조선 팔도를 여행하고자 하는 당신, 여행을 갈 때 어떻게 가겠습니까?",
    answer1: "발길 닿는대로~ 그때 그때 상황이 다를진데 뭐하러 계획하나?",
    answer1Code: "P",
    answer2: "일정을 계획적으로 짜 놔야 팔도를 다 볼 수 있어!",
    answer2Code: "J",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "7.	황산벌 전투의 신라군 지휘를 맡는 당신은 아군의 사기를 위해 친한 후배인 화랑을 희생해야 합니다. 이 때 당신의 선택은?",
    answer1: "내 후배를 어떻게 희생시켜? 난 못해",
    answer1Code: "F",
    answer2: "공과사는 구별해야 해! 부탁한다 관창!",
    answer2Code: "T",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "8.	당신은 선죽교 위에서 폭정을 하는 어리석은 왕을 몰아내고 더 좋은 세상을 만들자는 협박을 듣고 있습니다. 제안을 거절하면 죽을 것이고, 승낙하면 살 수 있습니다 어떻게 하겠습니까?",
    answer1:
      "내가 모시던 임금이지만 폭정을 참을 수가 없다! 더 좋은 세상을 만드는데 동참하겠어!",
    answer1Code: "T",
    answer2: "나는 왕을 위해 충성을 다해야 하는 신하이다 차라리 죽여라!",
    answer2Code: "F",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "9. 발명가인 나에게 오늘은 임금님이 백성들의 공평한 조세을 위해 방법을 고안해 오라는 명이 있었다 당신이라면 어떻게 해결하겠습니까?",
    answer1: "조세를 왜 발명가인 나한테…? 번지수 잘못 찾으셨어요",
    answer1Code: "S",
    answer2: "조세를 공평하게? 저한테만 맡겨 주십시오!",
    answer2Code: "N",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "10.	신라시대 왕족인 당신은 휴가를 떠납니다 어떤 휴가를 보내고 싶은가요?",
    answer1: "포석정에서 친우들과 떠들며 놀기",
    answer1Code: "E",
    answer2: "그냥 조용한게 좋아 집에서 놀래!",
    answer2Code: "I",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "11.	당신은 이번 인사 발령으로 두가지 부서를 선택할 수 있다. 첫번째는 행정 및 연구를 위한 홍문관, 두번째는 임금님의 일과에 대해 간쟁 등을 기록하는 사간원이다. 당신이라면 어떤 부서를 선택하겠습니까?",
    answer1:
      "여러 사람과 함께하는 것 보단 혼자 연구하는게 편해 홍문관으로 갈래!",
    answer1Code: "I",
    answer2: "여러 사람과 북적이며 일하는 것이 좋아 사간원으로 갈래!",
    answer2Code: "E",
    imgUrl: "./images/Q1.jpg",
  },
  {
    question:
      "12.어느날 한 노인이 다가와 쑥과 마늘을 100일동안 먹으면 소원을 들어준다고 하였다. 이때 당신의 선택은?",
    answer1: "먹는다고 소원이 이뤄지겠어, 포기한다.",
    answer1Code: "S",
    answer2: "밑져야 본전이야 열심히 먹는다.",
    answer2Code: "N",
    imgUrl: "./images/Q1.jpg",
  },
];

export const result = {
  ISTJ: { name: "신사임당", description: "" },
  ISFJ: { name: "황희", description: "" },
  INFJ: { name: "유관순", description: "" },
  INTJ: { name: "세종대왕", description: "" },
  ISTP: { name: "장영실", description: "" },
  ENFP: { name: "관창", description: "" },
  ENTP: { name: "율곡 이이", description: "" },
  ESTJ: { name: "태종,최승로", description: "" },
  ESFJ: { name: "서희", description: "" },
  ENFJ: { name: "정조", description: "" },
  ENTJ: { name: "이순신", description: "" },
  ISFP: { name: "이황", description: "" },
  INFP: { name: "김홍도", description: "" },
  INTP: { name: "정도전", description: "" },
  ESTP: { name: "김유신", description: "" },
  ESFP: { name: "장보고", description: "" },
};
